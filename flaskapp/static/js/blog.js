function showMenu() {
    $.getJSON( "/api/v1.0/themes", function( data ) {
        ...
        $.each( data.themes, function( key, val ) {
            ...
            data_slide++
            $(".carousel-inner").append(`<div class="carousel-item"><img class="first-slide" src="${val.img}" alt="First slide"><div class="container"><div class="carousel-caption text-left"><h1>${val.title_ru} <span class="badge badge-light">${val.summ}</span></h1><p>${val.description}</p><p><a class="btn btn-lg btn-primary" href="${val.title_slug}" role="button">Перейти</a></p></div></div></div>`)
